...
// nicht per ALTER TABLE erweiterbar sind. Ohne diesen Rebuild wäre
...
function init() {
  ...
  try {
    ...
    // versendet sein (order.fulfilled_by_skroutz im Skroutz-Order-Objekt).
    // Für Verpackungs-Compliance macht das keinen fachlichen Unterschied
    // (die Verpackung wird in beiden Fällen vom Händler in Verkehr
    // gebracht), aber Händler wollen die beiden Bestellarten trotzdem
    ...
    // juristische Volltext-Wiedergabe. label/criteria in allen 5 Dashboard-
    // Sprachen (de/en/fr/it/es) statt nur Italienisch - dashboard.html
    // wählt darüber direkt mit currentLang aus (Fallback auf de, falls für
    // eine spätere Ergänzung mal eine Sprache fehlen sollte). image
    // verweist auf ein aus der offiziellen CONAI-PDF extrahiertes Icon
    // (assets/conai/*.jpg im Frontend-Repo, per Fascia+Beschreibung von
    // Hand zugeordnet) - rein visuelle Ergänzung zum Text, kein Ersatz.
    ...
    // ONBOARDING_NICHES in dashboard.html, z. B. 'fashion', 'food_beverage',
    // 'home_deco') - bisher nur zum Vorbefüllen der Materialgewichte
    // benutzt und danach verworfen. Jetzt auf der SKU gespeichert, damit
    // der Klassifizierungs-Assistent (eco_fee_classification_guide_json)
    // später weiß, welche Kategorie für diesen Kunden relevant ist, ohne
    // dass er sie ein zweites Mal angeben muss. Frei/optional, bewusst
    ...
    db.prepare(`UPDATE countries SET eco_fee_classification_guide_json = ? WHERE code = 'IT'`).run(
      JSON.stringify({
        ...
        eintraege: [
          { fascia: 'A1.1', category: 'industrial', label: { de: 'Big Bags / Gewebesäcke für Industriezwecke', en: 'Big bags / industrial woven sacks', fr: 'Big bags / sacs tissés industriels', it: 'Big bag / sacchi tessuti per uso industriale', es: 'Big bags / sacos tejidos industriales' }, criteria: { de: 'Inkl. bereits integrierter Innenauskleidung (Liner).', en: 'Including any already-integrated liner.', fr: 'Y compris doublure intérieure déjà intégrée.', it: 'Incluso rivestimento interno (liner) già integrato.', es: 'Incluye forro interior ya integrado.' }, image: 'conai/big_bags.jpg' },
          { fascia: 'A1.1', category: 'beverage', label: { de: 'Wasserspender-Kanister (Bag-in-Box-Alternative)', en: 'Water dispenser bottles', fr: 'Bonbonnes pour fontaines à eau', it: 'Boccioni per erogatori d’acqua', es: 'Garrafas para dispensadores de agua' }, criteria: { de: 'Inkl. zugehöriger Verschlüsse.', en: 'Including caps.', fr: 'Bouchons inclus.', it: 'Inclusi i tappi.', es: 'Incluye tapones.' }, image: 'conai/water_dispenser_bottles.jpg' },
          { fascia: 'A1.1', category: 'general', label: { de: 'Industrie-/Agrar-Kisten und -Kästen (nicht geschäumt)', en: 'Industrial/agricultural crates (non-foam)', fr: 'Caisses et cageots industriels/agricoles (non expansés)', it: 'Cassette e casse industriali/agricole (non espanse)', es: 'Cajas y cajones industriales/agrícolas (no expandidos)' }, criteria: { de: 'Inkl. Deckel und Verschlusselemente.', en: 'Including lids and closure elements.', fr: 'Y compris couvercles et éléments de fermeture.', it: 'Inclusi coperchi ed elementi di chiusura.', es: 'Incluye tapas y elementos de cierre.' }, image: 'conai/industrial_crates.jpg' },
          { fascia: 'A1.1', category: 'general', label: { de: 'Paletten', en: 'Pallets', fr: 'Palettes', it: 'Pallet', es: 'Palés' }, criteria: { de: '', en: '', fr: '', it: '', es: '' }, image: 'conai/pallets.jpg' },
          { fascia: 'A1.1', category: 'general', label: { de: 'Kanister über 5 Liter Fassungsvermögen', en: 'Canisters over 5 litres capacity', fr: 'Bidons de plus de 5 litres', it: 'Taniche oltre 5 litri di capacità', es: 'Bidones de más de 5 litros de capacidad' }, criteria: { de: 'Inkl. zugehöriger Verschlüsse.', en: 'Including caps.', fr: 'Bouchons inclus.', it: 'Inclusi i tappi.', es: 'Incluye tapones.' }, image: 'conai/canisters_5l.jpg' },
          { fascia: 'A1.2', category: 'industrial', label: { de: 'Fässer und IBC-Container ab 30 Liter (HDPE)', en: 'Drums and IBC containers from 30 litres (HDPE)', fr: 'Fûts et conteneurs IBC à partir de 30 litres (HDPE)', it: 'Fusti e cisternette IBC da 30 litri (HDPE)', es: 'Bidones y contenedores IBC desde 30 litros (HDPE)' }, criteria: { de: 'Inkl. bereits integrierter Verschlüsse/Deckel.', en: 'Including already-integrated caps/lids.', fr: 'Y compris bouchons/couvercles déjà intégrés.', it: 'Inclusi tappi/coperchi già integrati.', es: 'Incluye tapones/tapas ya integrados.' }, image: 'conai/drums_ibc.jpg' },
          { fascia: 'A2', category: 'industrial', label: { de: 'Palettenfolie/-hauben, Schrumpffolie (Monopolymer PE)', en: 'Pallet wrap film/hoods, shrink film (mono-PE)', fr: 'Film/housses pour palettes, film rétractable (PE mono-matériau)', it: 'Film/cappucci per pallet, film termoretraibile (PE mono-materiale)', es: 'Film/capuchones para palés, film retráctil (PE monomaterial)' }, criteria: { de: 'Nicht metallisiert/lackiert.', en: 'Not metallised/coated.', fr: 'Non métallisé/laqué.', it: 'Non metallizzato/laccato.', es: 'No metalizado/lacado.' }, image: 'conai/pallet_wrap_film.jpg' },
          { fascia: 'B1.1', category: 'household', label: { de: 'HDPE-Flaschen/-Kanister bis 5L OHNE deckendes Etikett', en: 'HDPE bottles/canisters up to 5L WITHOUT covering label', fr: 'Bouteilles/bidons en HDPE jusqu’à 5L SANS étiquette recouvrante', it: 'Bottiglie/taniche in HDPE fino a 5L SENZA etichetta avvolgente', es: 'Botellas/bidones de HDPE hasta 5L SIN etiqueta envolvente' }, criteria: { de: 'Ohne mineralische Füllstoffe/"Carbon Black" (z. B. Reinigungsmittel, Waschmittel).', en: 'Without mineral fillers/"carbon black" (e.g. cleaning products, detergents).', fr: 'Sans charges minérales/« noir de carbone » (ex. produits nettoyants, lessives).', it: 'Senza cariche minerali/"carbon black" (es. detergenti, detersivi).', es: 'Sin cargas minerales/"carbon black" (p. ej. productos de limpieza, detergentes).' }, image: 'conai/hdpe_bottles_no_label.jpg' },
          { fascia: 'B1.1', category: 'beverage', label: { de: 'Fest verbundene Verschlüsse ("Tethered Caps") für Getränkeflaschen', en: 'Tethered caps for beverage bottles', fr: 'Bouchons attachés (« Tethered Caps ») pour bouteilles de boissons', it: 'Tappi legati ("Tethered Caps") per bottiglie di bevande', es: 'Tapones unidos ("Tethered Caps") para botellas de bebidas' }, criteria: { de: 'Für Getränkeverpackungen und Getränkekartons (CPL) jeder Fascia.', en: 'For beverage packaging and beverage cartons of any fascia.', fr: 'Pour emballages de boissons et cartons de boissons (CPL) de toute tranche.', it: 'Per imballaggi di bevande e cartoni per bevande (CPL) di qualsiasi fascia.', es: 'Para envases de bebidas y cartones de bebidas (CPL) de cualquier franja.' }, image: 'conai/tethered_caps.jpg' },
          { fascia: 'B1.2', category: 'beverage', label: { de: 'Klare PET-Flaschen/-Gläser OHNE deckendes Etikett', en: 'Clear PET bottles/jars WITHOUT covering label', fr: 'Bouteilles/pots en PET transparent SANS étiquette recouvrante', it: 'Bottiglie/vasetti in PET trasparente SENZA etichetta avvolgente', es: 'Botellas/tarros de PET transparente SIN etiqueta envolvente' }, criteria: { de: 'Transparent oder transparent eingefärbt, keine direkte Bedruckung statt Etikett.', en: 'Transparent or transparent-tinted, no direct printing instead of a label.', fr: 'Transparent ou teinté transparent, pas d’impression directe à la place d’une étiquette.', it: 'Trasparente o colorato trasparente, nessuna stampa diretta al posto dell’etichetta.', es: 'Transparente o teñido transparente, sin impresión directa en lugar de etiqueta.' }, image: 'conai/clear_pet_bottles_no_label.jpg' },
          { fascia: 'B2.1', category: 'household', label: { de: 'Starre PP-Behälter (Eimer, Becher, Schalen)', en: 'Rigid PP containers (buckets, cups, trays)', fr: 'Récipients rigides en PP (seaux, gobelets, barquettes)', it: 'Contenitori rigidi in PP (secchi, bicchieri, vaschette)', es: 'Envases rígidos de PP (cubos, vasos, bandejas)' }, criteria: { de: 'Ohne mineralische Füllstoffe/"Carbon Black".', en: 'Without mineral fillers/"carbon black".', fr: 'Sans charges minérales/« noir de carbone ».', it: 'Senza cariche minerali/"carbon black".', es: 'Sin cargas minerales/"carbon black".' }, image: 'conai/rigid_pp_containers.jpg' },
          { fascia: 'B2.2', category: 'household', label: { de: 'Starre PE-Behälter mit deckendem Etikett erlaubt', en: 'Rigid PE containers, covering label allowed', fr: 'Récipients rigides en PE, étiquette recouvrante autorisée', it: 'Contenitori rigidi in PE, etichetta avvolgente ammessa', es: 'Envases rígidos de PE, etiqueta envolvente permitida' }, criteria: { de: 'z. B. Eimer, Becher, Schalen.', en: 'E.g. buckets, cups, trays.', fr: 'P. ex. seaux, gobelets, barquettes.', it: 'Es. secchi, bicchieri, vaschette.', es: 'P. ej. cubos, vasos, bandejas.' }, image: 'conai/rigid_pe_containers_label.jpg' },
          { fascia: 'B1.2', category: 'beverage', label: { de: 'PET-Flaschen mit perforiertem, abnehmbarem Etikett', en: 'PET bottles with perforated, removable label', fr: 'Bouteilles en PET avec étiquette perforée et amovible', it: 'Bottiglie in PET con etichetta perforata e removibile', es: 'Botellas de PET con etiqueta perforada y desmontable' }, criteria: { de: 'Etikett muss laut Anleitung entfernbar sein.', en: 'Label must be designed to be removed per instructions.', fr: 'L’étiquette doit être conçue pour être retirée selon les instructions.', it: 'L’etichetta deve essere progettata per essere rimossa secondo le istruzioni.', es: 'La etiqueta debe estar diseñada para poder retirarse según las instrucciones.' }, image: 'conai/pet_perforated_label.jpg' },
          { fascia: 'B2.2', category: 'general', label: { de: 'Schutzelemente aus EPS (Styropor)', en: 'EPS (foam) protective elements', fr: 'Éléments de protection en EPS (polystyrène)', it: 'Elementi protettivi in EPS (polistirolo)', es: 'Elementos de protección de EPS (poliestireno)' }, criteria: { de: '', en: '', fr: '', it: '', es: '' }, image: 'conai/eps_protective.jpg' },
          { fascia: 'B2.2', category: 'general', label: { de: 'Mechanische Spender (Sprühpumpen, Trigger)', en: 'Mechanical dispensers (spray pumps, triggers)', fr: 'Distributeurs mécaniques (pompes vaporisateurs, gâchettes)', it: 'Dispenser meccanici (pompe spray, trigger)', es: 'Dispensadores mecánicos (bombas pulverizadoras, gatillos)' }, criteria: { de: '', en: '', fr: '', it: '', es: '' }, image: 'conai/mechanical_dispensers.jpg' },
          { fascia: 'B2.2', category: 'clothing', label: { de: 'Flexible PE-Folie für Kleidung (z. B. Kleiderhüllen)', en: 'Flexible PE film for clothing (e.g. garment film wrap)', fr: 'Film PE souple pour vêtements (ex. housses à vêtements)', it: 'Film flessibile in PE per abbigliamento (es. copriabiti)', es: 'Film flexible de PE para ropa (p. ej. fundas para prendas)' }, criteria: { de: 'Nicht metallisiert/lackiert, ohne mineralische Füllstoffe.', en: 'Not metallised/coated, without mineral fillers.', fr: 'Non métallisé/laqué, sans charges minérales.', it: 'Non metallizzato/laccato, senza cariche minerali.', es: 'No metalizado/lacado, sin cargas minerales.' }, image: 'conai/pe_film_clothing.jpg' },
          { fascia: 'B2.2', category: 'general', label: { de: 'Flexible PP-Beutel (z. B. für Nudeln, Süßwaren)', en: 'Flexible PP bags (e.g. for pasta, sweets)', fr: 'Sachets souples en PP (ex. pour pâtes, confiseries)', it: 'Sacchetti flessibili in PP (es. per pasta, dolciumi)', es: 'Bolsas flexibles de PP (p. ej. para pasta, dulces)' }, criteria: { de: '', en: '', fr: '', it: '', es: '' }, image: 'conai/pp_flexible_bags.jpg' },
          { fascia: 'B2.3', category: 'general', label: { de: 'PET-Tiefziehverpackungen (Schalen, Blister)', en: 'PET thermoformed packaging (trays, blister packs)', fr: 'Emballages thermoformés en PET (barquettes, blisters)', it: 'Imballaggi termoformati in PET (vaschette, blister)', es: 'Envases termoformados de PET (bandejas, blísteres)' }, criteria: { de: 'Transparent, nicht metallisiert.', en: 'Transparent, not metallised.', fr: 'Transparent, non métallisé.', it: 'Trasparente, non metallizzato.', es: 'Transparente, no metalizado.' }, image: 'conai/pet_thermoformed.jpg' },
          { fascia: 'B2.3', category: 'household', label: { de: 'Starre PS-Behälter (nicht geschäumt, z. B. Joghurtbecher)', en: 'Rigid PS containers (non-foam, e.g. yoghurt cups)', fr: 'Récipients rigides en PS (non expansés, ex. pots de yaourt)', it: 'Contenitori rigidi in PS (non espansi, es. vasetti di yogurt)', es: 'Envases rígidos de PS (no expandidos, p. ej. tarrinas de yogur)' }, criteria: { de: '', en: '', fr: '', it: '', es: '' }, image: 'conai/rigid_ps_containers.jpg' },
          { fascia: 'B2.3', category: 'general', label: { de: 'Netzbeutel (z. B. für Obst/Gemüse)', en: 'Net bags (e.g. for fruit/vegetables)', fr: 'Filets (ex. pour fruits/légumes)', it: 'Sacchetti a rete (es. per frutta/verdura)', es: 'Bolsas de red (p. ej. para fruta/verdura)' }, criteria: { de: 'Inkl. Zubehör (Banderolen/Etiketten), sofern ebenfalls PE/PP.', en: 'Including accessories (bands/labels), if also PE/PP.', fr: 'Y compris accessoires (bandes/étiquettes), si également en PE/PP.', it: 'Inclusi accessori (fascette/etichette), se anch’essi in PE/PP.', es: 'Incluye accesorios (bandas/etiquetas), si también son de PE/PP.' }, image: 'conai/net_bags.jpg' },
          { fascia: 'B2.3', category: 'clothing', label: { de: 'Kleiderbügel OHNE Metallteile', en: 'Coat hangers WITHOUT metal parts', fr: 'Cintres SANS parties métalliques', it: 'Grucce SENZA parti metalliche', es: 'Perchas SIN partes metálicas' }, criteria: { de: 'Für Kleidung, Wäsche und andere Waren.', en: 'For clothing, linen and other goods.', fr: 'Pour vêtements, linge et autres marchandises.', it: 'Per abbigliamento, biancheria e altre merci.', es: 'Para ropa, ropa de hogar y otras mercancías.' }, image: 'conai/hangers_no_metal.jpg' },
          { fascia: 'C', category: 'clothing', label: { de: 'Kleiderbügel MIT Metallteilen', en: 'Coat hangers WITH metal parts', fr: 'Cintres AVEC parties métalliques', it: 'Grucce CON parti metalliche', es: 'Perchas CON partes metálicas' }, criteria: { de: '', en: '', fr: '', it: '', es: '' }, image: 'conai/hangers_with_metal.jpg' },
          { fascia: 'C', category: 'clothing', label: { de: 'Kleidersäcke/-hüllen (Textil/Vlies)', en: 'Garment covers/dust covers (textile/non-woven)', fr: 'Housses à vêtements (textile/non tissé)', it: 'Sacchi/copriabiti (tessuto/tessuto-non-tessuto)', es: 'Fundas para prendas (textil/no tejido)' }, criteria: { de: 'Abweichende Merkmale von den günstigeren Fasce.', en: 'With characteristics differing from the cheaper fasce.', fr: 'Caractéristiques différentes des tranches moins chères.', it: 'Caratteristiche diverse rispetto alle fasce più economiche.', es: 'Características distintas a las de las franjas más económicas.' }, image: 'conai/garment_covers.jpg' },
          { fascia: 'C', category: 'general', label: { de: 'Jegliche PVC-Anteile (auch nur teilweise)', en: 'Any PVC content (even partial)', fr: 'Toute teneur en PVC (même partielle)', it: 'Qualsiasi contenuto di PVC (anche parziale)', es: 'Cualquier contenido de PVC (incluso parcial)' }, criteria: { de: 'Ausnahme: Fasce A1.1/A1.2.', en: 'Exception: fasce A1.1/A1.2.', fr: 'Exception : tranches A1.1/A1.2.', it: 'Eccezione: fasce A1.1/A1.2.', es: 'Excepción: franjas A1.1/A1.2.' }, image: 'conai/pvc_content.jpg' },
          { fascia: 'C', category: 'general', label: { de: 'Verpackungen mit "Carbon Black"-Pigment', en: 'Packaging containing "carbon black" pigment', fr: 'Emballages contenant du pigment « noir de carbone »', it: 'Imballaggi contenenti pigmento "carbon black"', es: 'Envases con pigmento "carbon black"' }, criteria: { de: 'Ausnahme: bestimmte Fälle in A1.1/A1.2/A2.', en: 'Exception: specific cases in A1.1/A1.2/A2.', fr: 'Exception : cas spécifiques en A1.1/A1.2/A2.', it: 'Eccezione: casi specifici in A1.1/A1.2/A2.', es: 'Excepción: casos específicos en A1.1/A1.2/A2.' }, image: 'conai/carbon_black.jpg' },
          { fascia: 'C', category: 'general', label: { de: 'Mehrschicht-Verpackungen aus verschiedenen Polymeren', en: 'Multi-layer packaging made of different polymers', fr: 'Emballages multicouches composés de polymères différents', it: 'Imballaggi multistrato composti da polimeri diversi', es: 'Envases multicapa compuestos por polímeros diferentes' }, criteria: { de: 'Nicht manuell trennbar.', en: 'Not manually separable.', fr: 'Non séparables manuellement.', it: 'Non separabili manualmente.', es: 'No separables manualmente.' }, image: 'conai/multilayer_packaging.jpg' },
          { fascia: 'C', category: 'beverage', label: { de: 'Flaschen mit Direktbedruckung statt Etikett', en: 'Bottles with direct printing instead of a label', fr: 'Bouteilles avec impression directe au lieu d’étiquette', it: 'Bottiglie con stampa diretta al posto dell’etichetta', es: 'Botellas con impresión directa en lugar de etiqueta' }, criteria: { de: '', en: '', fr: '', it: '', es: '' }, image: 'conai/direct_printed_bottles.jpg' },
          { fascia: 'C', category: 'beverage', label: { de: 'Fässer/Behälter des KEG-Typs (Bier, Wein)', en: 'KEG-type containers/drums (beer, wine)', fr: 'Fûts/conteneurs type KEG (bière, vin)', it: 'Fusti/contenitori tipo KEG (birra, vino)', es: 'Barriles/contenedores tipo KEG (cerveza, vino)' }, criteria: { de: '', en: '', fr: '', it: '', es: '' }, image: 'conai/keg_containers.jpg' }
        ]
      })
    );
